...
class KWPE(Layer):
    def __init__(self,bias=True,unit=16,**kwargs):
    ...
    def build(self, input_shape):
        ...
        self.we =self.add_weight(name='we',
                                      ...
                                      regularizer=l2(0.000001), 
                                       trainable=True)
        super(KWPE, self).build(input_shape)  # 一定要在最后调用它
    ...
    def call(self,x,mask=None):
        ...
        eij1 = K.dot(K.tanh(K.dot(pq,self.w2)+K.dot(pp,self.w3)+K.dot(x1,self.w1)),self.we)*sr
        ...
        eij2 = K.dot(K.tanh(K.dot(qp,self.w2)+K.dot(qq,self.w3)+K.dot(x2,self.w1)),self.we)*sr
        # eij2 = *self.sr
        ai2 = K.exp(eij2)
        if mask!=None:
